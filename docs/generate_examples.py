...
def generate_comparison_example():
    ...
    circular_params = HelixParams(
        x0=100.0, y0=100.0, radius=8.0, port_gap=0.5, tab_gap=0.4,
        angle_step=0.0, layer_index_list=[0, 1], track_width=0.5,
        connect_width=0.2, drill_size=0.2, via_size=0.4, net_number=1
    )
    
    rectangular_params = HelixRectangleParams(
        x0=200.0, y0=100.0, width=20.0, height=16.0, corner_radius=2.0,
        layer_index_list=[0, 1], track_width=0.5, connect_width=0.2,
        drill_size=0.2, via_size=0.4, net_number=1, port_gap=0.5, tab_gap=0.4
    )
    ...
    with open(os.path.join(images_dir, "comparison_circular_vs_rectangular.svg"), "w") as f:
        f.write(svg_content)
